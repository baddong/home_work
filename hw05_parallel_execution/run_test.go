...
func TestRun(t *testing.T) {
	...
	t.Run("func worked with N = 0", func(t *testing.T) {
		tasksCount := 10
		...
		require.Equal(t, nil, Run(tasks, 0, 10))
		require.Equal(t, int32(0), runTasksCount)
	})

	t.Run("Got ErrErrorsLimitExceeded if M = 0 ", func(t *testing.T) {
		tasksCount := 10
		tasks := make([]Task, tasksCount)
		...
		require.Equal(t, ErrErrorsLimitExceeded, Run(tasks, 10, 0))
		require.Equal(t, int32(10), runTasksCount)
	})
}
